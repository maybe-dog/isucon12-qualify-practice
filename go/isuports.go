...
func competitionRankingHandler(c echo.Context) error {
	...
	query :=
		`SELECT RANK() OVER (ORDER BY ps.score DESC, ps.row_num ASC) as rank, ps.score, ps.player_id, p.display_name, ps.row_num
		 FROM 
		 	(SELECT player_id, tenant_id, competition_id, MAX(row_num) as rn
		    FROM player_score
			WHERE tenant_id = (?) AND competition_id = (?)
			GROUP BY player_id) AS mps
		 INNER JOIN player_score AS ps
		 	ON mps.player_id = ps.player_id AND mps.rn = ps.row_num
			   AND mps.tenant_id = ps.tenant_id AND mps.competition_id = ps.competition_id
		 INNER JOIN player AS p ON ps.player_id = p.id
		 ORDER BY ps.score DESC, ps.row_num ASC LIMIT 100 OFFSET (?)`

	if err := tenantDB.SelectContext(ctx, &pagedRanks, query, tenant.ID, competitionID, rankAfter); err != nil {
		return fmt.Errorf("error retrieveCompetitionRanking faled: %w", err)
	}
